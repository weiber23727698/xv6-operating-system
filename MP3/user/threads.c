...
void schedule(void){
    ...
    if( is_thread_start == 0 )// execute the first thread in wait_queue at time==0 
        return ;
    else
    ...
    // ... implement RR here.
    int consume = threading_system_time-current_thread->start_run;
    if(current_thread->is_exited==1 || current_thread->is_yield==1 || consume == 3*TIME_QUANTUM_SIZE){
        // printf("ID is %d\n", current_thread->ID);
        current_thread->start_run = 0;
        current_thread = current_thread->next;
        current_thread->start_run = threading_system_time;
    }
    #endif
    ...
    if(current_thread->is_exited==1 || current_thread->is_yield==1 || is_thread_start==0){
        // printf("ID is %d\n", current_thread->ID);
        struct thread *t = current_thread->next;
        struct thread *choice = t;
        while(t != current_thread){
            if(t->remain_execution_time < choice->remain_execution_time)
                choice = t;
            else if(t->remain_execution_time==choice->remain_execution_time && t->ID < choice->ID)
                choice = t;
            t = t->next;
        }
        if(current_thread->is_exited == 0){
            if(current_thread->remain_execution_time < choice->remain_execution_time)
                choice = current_thread;
            else if(current_thread->remain_execution_time==choice->remain_execution_time && current_thread->ID < choice->ID)
                choice = current_thread;
        }
        current_thread = choice;
        // printf("choice is %d\n", choice->ID);
        // printf("remaing time is %d\n", choice->remain_execution_time);
    }
    #endif
