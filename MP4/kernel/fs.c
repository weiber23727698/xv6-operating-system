...
bmap(struct inode *ip, uint bn)
{
  ...
  for(int i = 1;i < 3;i++){
    if(bn < NDOUBLY_INDIRECT){
      ...
      a = (uint*)bp->data;
      // second layer
      uint second_layer = bn / NINDIRECT;
      if((addr = a[second_layer]) == 0){
        a[second_layer] = addr = balloc(ip->dev);
        log_write(bp);
      }
      brelse(bp);
      // find location in disk
      bp = bread(ip->dev, addr);
      a = (uint*)bp->data;
...
namex(char *path, int nameiparent, char *name, int depth)
{
  ...
  while((path = skipelem(path, name)) != 0){
    ...
    if(ip->type == T_SYMLINK){
      ...
      while(times<20 && ip->type==T_SYMLINK){
        // read the target path
        char route[MAXPATH];
        // int len = 0;
        // readi(ip, 0, (uint64)&len, 0, sizeof(int));
        // readi(ip, 0, (uint64)route, sizeof(int), len+1);
        if(readi(ip, 0, (uint64)route, 0, MAXPATH) < 0){
          iunlockput(ip);
          end_op();
          return 0;
        }
        ...
        char name2[DIRSIZ];
        if((ip = namex(route, 0, name2, depth + 1)) == 0){
          end_op();
          return 0;
        }
        ilock(ip);
